...
export default function ReportsPage() {
  ...
  const canDelete = ["ADMIN", "ANALYST"].includes(currentUserRole.toUpperCase());
  ...
  async function handleGenerate(data: {
    ...
    // Trigger #4: Analyst activity notification on report generation
    if (currentUser?.role?.toUpperCase() === "ANALYST") {
      notificationService.notifyRole(
        ...
        "ANALYST_ACTIVITY",
        "Analyst activity",
        `${currentUser.name} generated the ${data.name} report`
      );
    }
  }
      ...
      if (res.ok && data.report) {
        ...
        if (currentUser?.role?.toUpperCase() === "ANALYST") {
          notificationService.notifyRole(
            ...
            "ANALYST_ACTIVITY",
            "Analyst activity",
            `${currentUser.name} generated the Q3 VoC report`
          );
        }
      }
    } catch (err) {
...
<body>
  ...
  function handleDownload(report: Report) {
    // Triggers for Analyst & Viewer downloading report
    const currentUser = authService.getCurrentUser();
    ...
    if (roleUpper === "ANALYST") {
      notificationService.notifyRole(
        ...
        "ANALYST_ACTIVITY",
        "Analyst activity",
        `${currentUser?.name || "Analyst"} downloaded the ${report.name} report`
      );
      ...
      notificationService.notifyRole(
        ...
        "VIEWER_ACTIVITY",
        "Viewer activity",
        `${currentUser?.name || "Viewer"} downloaded the ${report.name} report`
      );
    }
